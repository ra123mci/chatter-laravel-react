...
type SvgTypes = 'twitter'|'facebook'|'instagram'|'linkedin'|'google'|'tiktok'|'add'|'add-circle'|'close'|'stats'|'bookmark'|'link'|'share'|'lupe'|
                'copy'|'arrow'|'arrow-dd'|'upload'|'page'|'pages'|'check-ok'|'check-cancel'|'warning'|'remove'|'lock'|'unlock'|'transfer'|'download'|'search'|
                'clock'|'edit'|'calendar'|'calendar-2'|'home'|'nominees'|'winners'|'collections'|'elements'|'courses'|'teachers'|'directory'|'jobs'|'bell'|
                'shop'|'articles'|'padlock'|'check'|'cart'|'play'|'play-2'|'user-login'|'user-add'|'user-avatar'|'mail'|'discount'|'eye'|'eye-2'|'submission'|
                'video'|'comment'|'image-add'|'image-remove'|'briefcase'|'chart'|'settings'|'billing'|'assets'|'dashboard'|'dashboard-3'|'alert'|'heart'|'heart-2'|
                "audio"|"paper-clip"|"smiley"|"picture"|"send";
export default function Svg({ type, className='', width='24' }:{ type:SvgTypes, className?:string, width?:string }){
    let size = 20; const changeViewBox = (w:number) => {size = w; return ''}

    const icons = <>
        {type === 'twitter' && <g id="twitter">
            <path d="M19.86 3.884a8.05 8.05 0 0 1-2.323.637 4.053 4.053 0 0 0 1.779-2.239 8.1 8.1 0 0 1-2.57.983 4.046 4.046 0 0 0-6.894 3.69 11.488 11.488 0 0 1-8.34-4.228 4.025 4.025 0 0 0-.547 2.035c0 1.403.714 2.642 1.8 3.367a4.034 4.034 0 0 1-1.833-.506v.051a4.05 4.05 0 0 0 3.246 3.967 4.068 4.068 0 0 1-1.828.07 4.05 4.05 0 0 0 3.78 2.809 8.117 8.117 0 0 1-5.99 1.675 11.45 11.45 0 0 0 6.202 1.818c7.442 0 11.511-6.165 11.511-11.511 0-.176-.003-.35-.011-.524a8.195 8.195 0 0 0 2.018-2.094z"/>
        </g>}
        {type === 'facebook' && <g id="facebook">
        ...
        {type === 'add-circle' && <g id="add-circle">
        ...
        {type === 'check-cancel' && <g id="check-cancel">
        ...
        {type === 'calendar-2' && <g id="calendar-2">
        ...
        {type === 'collections' && <g id="collections">
        ...
        {type === 'user-login' && <g id="user-login">
        ...
        {type === 'user-avatar' && <g id="user-avatar">
        ...
        {type === 'submission' && <g id="submission">
        ...
        {type === 'image-remove' && <g id="image-remove">
        ...
        {type === 'dashboard-3' && <g id="dashboard-3">
            <path d="M0,10C0,4.5,4.5,0,10,0s10,4.5,10,10c0,3.3-1.6,6.3-4.1,8.1l-1.4-1.4c2.2-1.4,3.6-3.9,3.6-6.7c0-4.4-3.6-8-8-8 s-8,3.6-8,8s3.6,8,8,8c0.9,0,1.7-0.1,2.5-0.4l1.6,1.5c-1.2,0.5-2.6,0.8-4,0.8C4.5,20,0,15.5,0,10z M13.5,7V5.9h-2V7h-1v2h1v5.1h2V9 h1V7H13.5z M6.5,13v1.1h2V13h1v-2h-1V5.9h-2V11h-1v2H6.5z"/>
        </g>}
        {type === 'alert' && <g id="alert">
